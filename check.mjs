...
await page.locator('#rotate').click();const autoStart=await page.evaluate(()=>window.__busStudio.camera.position.toArray());await page.waitForFunction(start=>JSON.stringify(window.__busStudio.camera.position.toArray())!==JSON.stringify(start),autoStart);const autoEnd=await page.evaluate(()=>window.__busStudio.camera.position.toArray());
await page.setViewportSize({width:390,height:844});await page.locator('#reset').click();await page.waitForTimeout(1200);await page.screenshot({path:'/tmp/bus-mobile.png'});
console.log(JSON.stringify({errors,orbitChanged:JSON.stringify(before)!==JSON.stringify(orbit),zoomDistance:zoom,autoRotates:JSON.stringify(autoStart)!==JSON.stringify(autoEnd),canvas:await page.locator('canvas').count(),mobileOverflow:await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),stats:await page.locator('.stats').innerText(),darkBackground:await page.evaluate(()=>window.__busStudio.scene.background.getHexString())},null,2));await browser.close();
