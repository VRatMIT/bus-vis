...
const sun=new THREE.DirectionalLight(0xfff8ea,1.3);sun.position.set(-3,10,6);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-7,right:7,top:6,bottom:-6,near:.5,far:30});sun.shadow.normalBias=.025;sun.shadow.bias=-.00015;sun.shadow.radius=4;scene.add(sun);
const fill=new THREE.HemisphereLight(0xe1eefc,0xa4aa91,1.1);scene.add(fill);
const floorGrid=createFloorGrid();scene.add(floorGrid.terrain,floorGrid.mesh);
